...
export default function Header() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 px-4 pt-4">
      <header
        className={`mx-auto max-w-[1140px] rounded-2xl transition-all duration-300 ${
          scrolled
            ? 'bg-white/85 backdrop-blur-md shadow-lg border border-white/60'
            : 'bg-white/70 backdrop-blur-sm shadow-md border border-white/40'
        }`}
      >
        <div className="flex items-center justify-between h-14 px-5">
          <Link href="/" className="flex-shrink-0">
            <Image src="/images/basis_logo.svg" alt="de Basis" width={110} height={36} priority />
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-5">
            ...
            <Link
              href="/contact/"
              className="bg-primary text-black text-sm font-medium px-4 py-2 rounded-btn hover:bg-primary-hover transition-colors ml-2"
            >
              Boek bezichtiging
            </Link>
          </nav>
        ...
        {/* Mobile menu */}
        {open && (
          <div className="lg:hidden border-t border-black/10 px-5 py-4 flex flex-col gap-3">
            {navLinks.map(link => (
              <Link
                key={link.href}
                href={link.href}
